...
inline float pMap(const float& iValue, const float& iInStart, const float& iInStop, const float& iOutStart, const float& iOutStop)
{
	...
	void runGeneration()
	{
		if( mFitnessFunction ) {
			...
			// Handle mating:
				...
				for(int i = 0; i < mPopulationSize; i++) {
					int tN = pMap( tScores[i], tWorstScore, tBestScore, 1.0f, 100.0f );
					for(int j = 0; j < tN; j++) {
